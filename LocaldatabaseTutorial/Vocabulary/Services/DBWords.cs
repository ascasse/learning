...
namespace Vocabulary.Services
{
    ...
    public class DBWords : IDisposable
    {
        ...
        {
            List<Category> categories = await Connection.Table<Category>().ToListAsync();
            foreach (Category ctg in categories)
            {
                ctg.Words = await Connection.QueryAsync<Word>("SELECT * FROM Words WHERE categoryId = ?", new object[] { ctg.Id });
            }
            return categories;
